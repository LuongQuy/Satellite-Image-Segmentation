...
for i, img_id in enumerate(img_id_list):
    ...
    img_size = skimage.io.imread(data_path+'sixteen_band/'+img_id+'_P.tif', plugin="tifffile").shape
    pdict = get_polygon_dict(img_id, Class_dict, img_size, wkt_df, grid)
    masks[img_id] = get_polygons_masks(pdict, order_dict, img_size, filename=data_path+'masks/'+img_id+'_mask.tif')
