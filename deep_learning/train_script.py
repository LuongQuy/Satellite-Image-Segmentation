...
# load the test set
test_df = load_sample_df(data_path+'test_samples_small.csv', class_type=class_type, others_frac=non_class_fraction, seed=1)

# the train and validation datasets
train_set = dataset(train_df, img_path, mask_path, class_position[class_type], augment=augment_data, crop_size=crop_size)
val_set = dataset(val_df, img_path, mask_path, class_position[class_type], augment=augment_data, crop_size=crop_size)
test_set = dataset(test_df, img_path, mask_path, class_position[class_type], augment=augment_data, crop_size=crop_size)
...
checkpoint_name = output_path+model_name+'_checkpoint.pth'
try:
...
for epoch in range(nb_epochs_finished, n_epoch):
    ...
    if best_F1 < log_train["f1_val"]["mean"][-1]:
        ...
        torch.save(best_model, output_path+model_name+'_best.pickle')
        best_F1 = log_train["f1_val"]["mean"][-1]

# Save the log of training
with open(output_path+model_name+'_log_train.pickle', 'wb') as handle:
    pickle.dump(log_train, handle, protocol=pickle.HIGHEST_PROTOCOL)
print('\n>>> LOG saved on disk at'+output_path+model_name+'_log_train.pickle')

# Save trained model state dict
torch.save(model.state_dict(), output_path+model_name+'_trained.pt')
...
# save the scores
with open(output_path+model_name+'_scores.pickle', 'wb') as handle:
    pickle.dump(all_scores, handle, protocol=pickle.HIGHEST_PROTOCOL)
print('\n>>> Scores saved on disk at'+output_path+model_name+'_scores.pickle')
